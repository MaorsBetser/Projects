...
void record_preferences(int ranks[])
{
    ...
    for(int i = 0; i < candidate_count; i++)
    {
        ...
        {
            int winner_pos = ranks[i];
            int loser_pos = ranks[j];
            preferences[winner_pos][loser_pos] += 1;
        }
    }
...
void sort_pairs(void)
{
    for(int i = 0; i < pair_count; i++)
    {
        ...
        {
            if(preferences[pairs[j].winner][pairs[j].loser] > preferences[strongest_pair.winner][strongest_pair.loser])
            {
                index = j;
                strongest_pair = pairs[j];
                printf("strongest_pair: winner: %i loser: %i \n",strongest_pair.winner, strongest_pair.loser);
            }
        }
        pair temp_pair = pairs[i];
        pairs[i] = strongest_pair;
